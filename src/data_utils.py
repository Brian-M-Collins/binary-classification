...
def undersample_majority(df, percent_conserve):
    ...
    ('label'==0) by conserving a given percent of the majority class.
    Inputs:
        - df:  A pandas DataFrame with columns:
               - 'text' containing strings of text.
    ...
    clean_texts = df[df["label"] == 0]
    toxic_texts = df[df["label"] == 1]
...
def augment_text(df, aug, num_threads, num_times):
    ...
    aug_dict = {"text": [], "label": to_augmentY}
